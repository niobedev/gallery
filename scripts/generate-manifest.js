...
function parseEncFile(filePath: string): { content: string; metadata: MediaFile } {
...
function scanDirectory(dirPath: string, type: 'videos' | 'pictures'): Record<string, MediaFile[]> {
...
function generateId(type: string, month: string, index: number): string {
  const num = String(index + 1).padStart(3, '0');
  return `${type.charAt(0)}-${month}-${num}`;
}

function assignIds(files: MediaFile[], type: 'videos' | 'pictures', month: string): void {
